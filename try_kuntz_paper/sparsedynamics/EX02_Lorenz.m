...
load("traces.mat")
x= traces(1,:);
new_indices = mod((1:numel(x)) - 1 - 1, numel(x)) + 1;
new_indices2 = mod((1:numel(x)) - 2 - 1, numel(x)) + 1;
y = x(new_indices(1:end-2));
z = x(new_indices2(1:end-2));
x = x(1:end-2);
polyorder = 2;
usesine = 0;
...
x0=[0.1; 1; 0.1];  % Initial condition
